...
export default function Login() {
	...
	// Demo native login (keeps previous behavior)
		...
		setLoading(true);
		try {
				// Expect backend route /auth/:provider/start to redirect or return a URL. Include action=signup when signing up.
				const search = mode === 'signup' ? '?action=signup' : '';
				const res = await fetch(`/auth/${provider}/start${search}`, { method: 'GET' });
			if (res.ok) {
				const json = await res.json().catch(() => null);
				if (json && json.url) {
					// redirect the browser to provider auth URL
		...
		// Fallback: demo sign-in to keep UX smooth during local development
				setStatus('OAuth not configured — falling back to server-side flow (if available)');
				try {
					if (mode === 'signup') {
					...
					setStatus(err.message || 'Fallback auth failed');
				}
				setLoading(false);
	}
		...
		return (
			...
			<div className='space-y-4'>
				<div className='flex flex-col gap-3'>
					<button onClick={() => startOAuth('google')} disabled={loading} className='flex items-center justify-center gap-3 w-full px-4 py-2 bg-white text-slate-900 rounded shadow-sm hover:brightness-95'>
					...
					<button onClick={() => startOAuth('linkedin')} disabled={loading} className='flex items-center justify-center gap-3 w-full px-4 py-2 bg-blue-700 text-white rounded shadow-sm hover:brightness-110'>
				...
				<form onSubmit={handleLocalLogin} className='space-y-3'>
					...
					<button type='submit' disabled={loading} className='w-full py-2 px-4 bg-emerald-600 rounded text-white'>{loading ? 'Signing in…' : 'Sign in'}</button>
				</form>
